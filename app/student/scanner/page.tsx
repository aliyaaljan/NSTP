...
*/
  ...
  // Card click filter
  const handleCardClick = (filterType: "present" | "not-scanned") => {
  ...
  const getResponsivePadding = () => {
    ...
    if (isTablet) {
      const tabletRailWidth = Math.max(
        70,
        COLLAPSED_W * (window.innerWidth / 768)
      )
      return {
  ...
  const baseFilteredScans = filterScansByMonthAndWeek(
    scans,
    selectedMonth,
    selectedWeek
  )
  ...
  // calculations
  const totalScans = baseFilteredScans.length
  const presentCount = baseFilteredScans.filter(
    (s) => s.status === "Present"
  ).length
  const totalHours = baseFilteredScans.reduce(
  ...
  const scannedNames = new Set(baseFilteredScans.map((s) => s.name))
  const notScannedNames = classRoster.filter((name) => !scannedNames.has(name))
  const notScannedCount = notScannedNames.length

  let filteredScans = baseFilteredScans
  if (statusFilter) {
    filteredScans = filteredScans.filter((scan) => {
      if (statusFilter === "present") return scan.status === "Present"
      if (statusFilter === "not-scanned") return false // handled entirely by passing notScannedNames to the Panel
      return true
    })
  }
    ...
    >
      ...
      >
        ...
        <ScanLogPanel
          ...
          selectedWeek={selectedWeek}
          notScannedNames={
            statusFilter === "not-scanned" ? notScannedNames : []
          }
        />
