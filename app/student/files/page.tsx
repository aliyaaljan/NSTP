...
const forms: Form[] = [
  { 
    ...
    fileType: "PDF",
    submittedFiles: [
      { name: "community_service_log.pdf", type: "PDF", size: "2.4 MB", url: "/sample.pdf" }
    ],
    submittedLinks: ["https://docs.google.com/spreadsheets/d/example"]
  },
  { id: "2", name: "Project Proposal Form", deadline: "July 17, 2026", status: "pending" },
  { id: "3", name: "Progress Report Form", deadline: "July 24, 2026", status: "pending" },
  { id: "4", name: "Final Evaluation Form", deadline: "July 30, 2026", status: "pending" },
];
...
function FormsTable({ forms }: { forms: Form[] }) {
  ...
  const stats = [
    { label: "Submitted", value: uploadedCount, icon: "ti-circle-check", color: submittedColor },
    { label: "Pending", value: totalCount - uploadedCount, icon: "ti-clock", color: pendingColor },
  ];

  // Filter and search
  const filteredForms = forms.filter(form => {
    if (activeFilter !== "All" && form.status !== (activeFilter === "Submitted" ? "uploaded" : "pending")) {
      return false;
    }
    // Search filter
    if (searchQuery.trim() !== "") {
  ...
  const filteredTotalPages = Math.max(1, Math.ceil(sortedForms.length / pageSize));
  ...
  const getFileType = (file: File) => {
    ...
    if (name.endsWith(".jpg") || name.endsWith(".jpeg") || name.endsWith(".png") || name.endsWith(".gif") || name.endsWith(".webp")) return "IMG";
    ...
    <>
      ...
      <div className="sf-adv-table-card">
        ...
        <div className="sf-adv-table-wrapper">
          {sortedForms.length === 0 ? (
            ...
            <table className="sf-adv-table">
              <thead>
                <tr>
                  <th style={{ width: "45%" }} onClick={() => handleSort("name")}>
                    File
                    {getSortIcons("name")}
                  </th>
                  <th 
                  ...
                  <tr key={form.id}>
                    <td>
                      <div className="sf-form-name">
                        <IconFileText size={16} stroke={1.75} className="sf-form-icon" />
                        {form.name}
                      </div>
                    </td>
        ...
        <div className="sf-adv-pagination">
          <div className="sf-adv-pagination-info">
            Showing {sortedForms.length === 0 ? 0 : (currentPage - 1) * pageSize + 1}–
            {Math.min(currentPage * pageSize, sortedForms.length)} of {sortedForms.length} forms
          </div>
          <div className="sf-adv-pagination-controls">
            ...
            {Array.from({ length: Math.min(filteredTotalPages, 5) }, (_, i) => i + 1).map(p => (
              <button
                key={p}
            ...
            <button
              className="sf-adv-page-btn"
              disabled={currentPage === filteredTotalPages || sortedForms.length === 0}
        ...
        <div className="sf-modal-backdrop" onClick={() => setShowModal(false)}>
          <div className="sf-modal" onClick={(e) => e.stopPropagation()}>
            <div className="sf-modal-header">
              <span className="sf-modal-title">Upload: {selectedForm.name}</span>
            ...
            <div className="sf-modal-body">
              <div className="sf-modal-content">
                ...
                {showLinkInput && (
                  <div className="sf-link-input-container">
                    ...
                    <button 
                      ...
                      onClick={addLink}
                      disabled={!linkInput.trim() || !linkInput.trim().startsWith("http")}
                    >
                      <IconPlus size={14} stroke={2} style={{ color: '#FFFFFF' }} />
                      Add
                    </button>
                    <button 
                      ...
                      <div key={index} className="sf-file-preview-card">
                        ...
                        <div className="sf-file-preview-info">
                          ...
                          <div className="sf-file-preview-meta">
                            <span className="sf-file-preview-type">{getFileType(file)}</span>
                            <span className="sf-file-preview-size">{formatFileSize(file.size)}</span>
                          </div>
                        </div>
                        <button 
                ...
                {links.length > 0 && (
                  <div className="sf-file-grid">
                    {links.map((link, index) => (
                      <div key={index} className="sf-file-preview-card">
                        <div className="sf-file-preview-icon-wrapper" style={{ background: '#E8EDE5', color: '#1B4332' }}>
                          <IconLink size={20} stroke={1.75} />
                        </div>
                        <div className="sf-file-preview-info">
                          <div className="sf-file-preview-name">{link}</div>
                          <div className="sf-file-preview-meta">
                            <span className="sf-file-preview-type" style={{ background: '#1B4332' }}>LINK</span>
                            <span className="sf-file-preview-size">{link.replace(/^https?:\/\//, '').split('/')[0]}</span>
                          </div>
                        </div>
                        <button 
                          className="sf-file-preview-remove"
                ...
                {selectedFiles.length === 0 && links.length === 0 && !showLinkInput && (
      ...
      {showViewModal && viewingForm && (
        <div className="sf-modal-backdrop" onClick={() => setShowViewModal(false)}>
          <div className="sf-modal" onClick={(e) => e.stopPropagation()}>
            <div className="sf-modal-header">
              <span className="sf-modal-title">Submitted: {viewingForm.name}</span>
            ...
            <div className="sf-modal-body">
              <div className="sf-modal-content">
                {/* Submitted Files */}
                {viewingForm.submittedFiles && viewingForm.submittedFiles.length > 0 && (
                  <div className="sf-file-grid">
                    {viewingForm.submittedFiles.map((file, index) => (
                      <div key={index} className="sf-file-preview-card">
                        <div className="sf-file-preview-icon-wrapper">
                          <IconFile size={20} stroke={1.75} style={{ color: '#1B4332' }} />
                        </div>
                        <div className="sf-file-preview-info">
                          <div 
                            className="sf-file-preview-name" 
                            style={{ cursor: 'pointer', color: '#1B4332', textDecoration: 'underline' }}
                            onClick={() => file.url && window.open(file.url, '_blank')}
                          >
                            {file.name}
                          </div>
                          <div className="sf-file-preview-meta">
                            <span className="sf-file-preview-type">{file.type}</span>
                ...
                {/* Submitted Links */}
                {viewingForm.submittedLinks && viewingForm.submittedLinks.length > 0 && (
                  <div className="sf-file-grid">
