...
import { createGpsSite } from "@/lib/admin/settings-actions"
import {
  emptyGpsSiteCreatePayload,
  validateGpsSiteCreatePayload,
  type GpsSiteCreatePayload,
} from "@/lib/admin/settings-edit"
import type { GpsSectionOption } from "@/lib/admin/settings"
...
export default function AddGpsSiteModal({
  ...
  const [form, setForm] = useState<GpsSiteCreatePayload>(emptyGpsSiteCreatePayload())
  ...
  const reset = useCallback(() => {
    setForm(emptyGpsSiteCreatePayload())
    setError(null)
  }, [])
  ...
  function patchForm(updates: Partial<GpsSiteCreatePayload>) {
    setForm((prev) => ({ ...prev, ...updates }))
  }

  function handleAdd() {
    const validationError = validateGpsSiteCreatePayload(form)
    if (validationError) {
    ...
    startTransition(async () => {
      const result = await createGpsSite(form)
      if (!result.ok) {
